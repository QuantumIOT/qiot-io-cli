...
function CmdContext(){
  ...
  if (self.options.debug) self.logger.debugging = true;

  if (self.options.csv || self.options.tsv) self.options.raw = true;

  self.goodCallback = function(error,data) { if (error) self.logger.error(error,data); };
}
...
CmdContext.prototype.ensureOptions = function(specs){
  ...
  return new Promise(function(resolve,reject){
    ...
    function nextSpec(){
      ...
      self.ensureOption(spec).then(nextSpec).catch(reject);
    }
    nextSpec();
  });
};

CmdContext.prototype.dumpObject = function(object,sortFields){
  var self = this;

  if (self.options.silent) return;
  if (self.options.json) return self.logger.consoleLOG(self.options.raw ? JSON.stringify(object) : JSON.stringify(object,null,2));

  var rows = _.toPairs(flat.flatten(object)).map(function(pair){ pair[0] = self.boldString(pair[0]); return pair; });

  if (sortFields) rows = _.sortBy(rows,function(pair){ var key = pair[0]; return [key.split('.').length,key]; });
...
CmdContext.prototype.dumpTable = function(fields,objects){
  ...
  if (self.options.silent) return;
  if (self.options.json) return self.logger.consoleLOG(self.options.raw ? JSON.stringify(objects) : JSON.stringify(objects,null,2));
...
function commonStrings(value,callback){
  ...
  if (_.isObject(value))    return callback(JSON.stringify(value));
...
CmdContext.prototype.bestThingToken = function(thingToken){
  var self = this;

  if (thingToken !== TOKEN_PLACEHOLDER) return thingToken;

  return self.config.settings[CmdContext.CURRENT_OPTION_PREFIX + CmdContext.THING_OPTION] || thingToken;
};
