...
API.addDefn(null,          'post',SIGNIN_URL,           true, ['email','password'],   true, 'signin',     HOST.allCodes.OK,       'token',                       null);
...
API.addDefn(HOST.AUTH_ACCT,'post',REGISTER_URL,         false,[],                     true ,'register',   HOST.allCodes.OK,        null,                         null);
...
API.addDefn(HOST.AUTH_USER,'get', MESSAGES_URL,         false,['thing_token','limit'],false,'messages',   HOST.allCodes.OK,        ['messages','binaryMessages'],[MESSAGE_FIELDS,MESSAGE_FIELDS]);
        ...
        if (defn.expect_data.length == 0)
          ...
          _.each(dataFound,function(dataKey,index){
            if (dataKey === 'token')
              ...
              var fields  = defn.table_fields && defn.table_fields[index];
              if (!fields)
                cmd.dumpObject(data);
